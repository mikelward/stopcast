...
Trackmo is TfL-specific today: the data layer talks only to the TfL Unified API, and
...
This is the load-bearing point: **even if trackmo supported other cities, London would
...
- **It's free and keyless.** ~50 req/min without a key, ~500 with a user `app_key` — well
  inside trackmo's usage (SPEC *Cost and reliability*).
- **It keeps trackmo client-only.** No backend is needed to serve London. GTFS on a network
...
- **GTFS-Realtime (protobuf over HTTP)** — three optional entity types:
  ...
  - **Service alerts** — disruptions. **Optional**, and some operators publish them through a
    separate alerts API instead. Trackmo's honesty floor (warn about a closed line or stop
...
**v1 would show live predictions only, never a scheduled fallback** — matching trackmo's
...
- **Telling them apart** — a sketch of the *principle*, not a GTFS-Realtime spec; the exact
  ...
  clears the same honesty floor trackmo already applies to a stale TfL snapshot — a
...
- **If we ever added a scheduled fallback (post-v1), how to show it in the UI** so it can't
  ...
  - **Muted and marked.** Scheduled rows grayed / lower-contrast with a small "scheduled" (or
    timetable "~") marker, sorted **below** the live rows — reusing trackmo's existing
...
1. **Client-only** (matches trackmo today, no backend). Download the static Schedule to build
   ...
   - *Privacy / Data Safety:* the Schedule and RT feed downloads leave the device and reach
     ...
     trackmo's Play Data Safety declaration (Data Safety is about data types collected or
     shared, not "network access", which is just the `INTERNET` manifest permission); the
     provider sees only the request and its network metadata. **Baseline:** trackmo is not
     ...
     forks #2/#3, and unlike trackmo's existing TfL calls.
2. **Add a backend.** Ingest GTFS + RT server-side and expose trackmo's own per-stop arrivals
   ...
   - *Privacy / Data Safety:* a **new recipient** — a server we run — now sees which stops
     users query (and, for nearby, coordinates). trackmo already sends that data to TfL, but
     routing it to our own server is a **Play Data Safety change** and a departure from
     trackmo's **client-only** posture (a server we operate, not just a third-party API we
     call).
3. **Use an aggregator API** (e.g. Transitland, or a paid transit API) that already exposes
   ...
   - *Reliability:* their uptime and rate limits become trackmo's.
...
The honest summary: fork #1 keeps trackmo a **client-only app**; forks #2 and #3 make it a
...
The domain layer (`app.trackmo.domain`) is *shaped* around one departures model much of a
...
(Pill rendering is in the UI layer, `app.trackmo.ui.LinePill`, not the domain.)
...
- **No crowd-sourced live locations.** Some apps blend their own users' reported positions
  ...
  collection service, a far heavier Play Data Safety profile than trackmo's on-demand,
  one-shot coordinate to TfL (see the baseline above — trackmo is not "location stays on the

...
import app.trackmo.domain.WidgetRefresh
...
internal const val WIDGET_REFRESH_WORK = "trackmo-widget-refresh"
 ...
 *  widget is removed ([TrackmoWidget.onDelete]). Awaited for the same reason as the enqueue. */
...
/** True when at least one [TrackmoWidget] is installed on a host. A refresh cycle with none
...
private suspend fun anyWidgetInstalled(context: Context): Boolean =
    GlanceAppWidgetManager(context.applicationContext)
        .getGlanceIds(TrackmoWidget::class.java).isNotEmpty()
 ...
 * chain isn't pushed back on every render. Armed from [TrackmoWidget.provideGlance] (the render

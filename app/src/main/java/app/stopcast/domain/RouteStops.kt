...
object RouteStops {
    ...
    fun ahead(
        ...
        branch: String?,
        bus: Boolean = false,
    ): List<RouteStop>? = (resolve(sequence, stopId, destination, branch, bus) as? Resolution.Found)?.stops
    ...
    fun resolve(
        ...
        return Resolution.Found(path.map { id -> RouteStop(id, sequence.stopNames[id].orEmpty()) })
    }

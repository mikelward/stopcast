...
object StationMatcher {
     ...
     * "KSX", "910GKGX" → "KGX"), or the whole id when it has none of those prefixes.
     */
    internal fun codeOf(id: String): String {
        val upper = id.uppercase()
        val prefix = CODE_PREFIXES.firstOrNull { upper.startsWith(it) && upper.length > it.length }
    ...
    // TfL id prefixes ahead of a station's code: interchanges (HUB…), tube/DLR/Overground/
    // Elizabeth line stations (940GZZLU/940GZZDL/…), National Rail (910G…). Longest first.
    private val CODE_PREFIXES = listOf("940GZZLU", "940GZZDL", "940GZZCR", "940GZZ", "910G", "HUB")
}

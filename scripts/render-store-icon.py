...
the blue line converging into a white arrow) cropped to the 72dp safe zone so the
...
d.line([X(64), Y(54), X(84), Y(54)], fill=WHITE, width=w)                          # shaft (flat ends)
d.line([X(77), Y(47), X(84), Y(54), X(77), Y(61)], fill=WHITE, width=w, joint="curve")  # head
for (x, y) in [(77, 47), (77, 61), (64, 54)]:                                      # round head tips + tail
    r = w / 2
    d.ellipse([X(x) - r, Y(y) - r, X(x) + r, Y(y) + r], fill=WHITE)
